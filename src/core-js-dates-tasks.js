...
function getNextFriday(date) {
  const a = new Date(`${date}`);
  const dateNew = new Date(a.getTime());
  const nextFriday = new Date(
    dateNew.setDate(dateNew.getDate() + ((7 - dateNew.getDay() + 5) % 7 || 7))
  );
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const differenceInMilliseconds = Math.abs(b - a);
...
function isDateInPeriod(date, period) {
  ...
  return check.getTime() <= end.getTime() && check.getTime() >= start.getTime();
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const getTot = daysInMonth(d.getMonth() + 1, d.getFullYear());
  const sat = [];
  const sun = [];
  for (let i = 1; i <= getTot; i += 1) {
